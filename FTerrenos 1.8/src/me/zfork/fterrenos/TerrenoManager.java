...
public class TerrenoManager {
	...
	static HashMap<Flag<?>, Object> flags = new HashMap<Flag<?>, Object>();
	...
	public static void loadSchematic(World world, File file, Vector v){
		try{
			...
			cc.paste(es, v, false);
			es.flushQueue();
		}catch(IOException | MaxChangedBlocksException | com.sk89q.worldedit.world.DataException e){
			e.printStackTrace();
		}
	}
	...
	public static int GetAreasStaff(Player p, String pl){
		...
		RegionManager regionmanager = regioes.get(p.getWorld());
		Map<String, ProtectedRegion> mgr = regionmanager.getRegions();
		LocalPlayer player = wg.wrapOfflinePlayer(instance.getServer().getOfflinePlayer(pl));
		for(ProtectedRegion region : mgr.values()){
			if(region.isOwner(player)) {
				i++;
			}
		...
		try{
			...
			if(Existe(p, area, p.getWorld().getName())){
				...
				if(!regions2.isOwnerOfAll(player)){
					...
					instance.econ.withdrawPlayer(p, tamanho * instance.getConfig().getInt("Config.Preco_Bloco"));
	...
	public static void PvP(Player p, String pvp){
		...
		if(region.isOwner(player)){
			try{
				...
				instance.econ.withdrawPlayer(p, instance.getConfig().getInt("Config.Preco_PvP"));
				regionManager.save();
			}catch(Exception e){
				e.printStackTrace();
			}
	...
	public static void Mob(Player p, String mob){
		...
		if(region.isOwner(player)){
			try{
				...
				instance.econ.withdrawPlayer(p, instance.getConfig().getInt("Config.Preco_Mob"));
				regionManager.save();
			}catch(Exception e){
				e.printStackTrace();
			}
