...
public class PagSeguro extends Thread{
	...
	public PagSeguro(FVip instance, String codigo, Player p){
		this.instance = instance;
		this.sqlite = instance.getSQLite();
		this.codigo = codigo.toUpperCase();
		this.p = p;
		try {
			this.transaction = TransactionSearchService.searchByCode(new AccountCredentials(instance.getConfig().getString("Config.PagSeguro.Email"), instance.getConfig().getString("Config.PagSeguro.Token")), codigo);
	...
	public void run(){
		if(transaction == null){
			...
			return;
		}else if(transaction.getStatus().getValue() != 4 || transaction.getStatus().getValue() != 3){
		...
		for(Object item : transaction.getItems()) {
			...
			if(desc.matches("[a-zA-Z0-9]+,\\d+")) itens.add(item.toString());
		}
		if(itens.size() == 0){
		...
		try{
			...
			PlayerActiveVipEvent event = new PlayerActiveVipEvent(p, ActivationType.PAGSEGURO, codigo, grupo, new Date(Long.parseLong(tempo)));
			...
			if(!event.isCancelled()){
				...
				if(instance.getCabecas().contains("Cabecas") && instance.getConfig().getBoolean("Config.Mural_Cabecas.PagSeguro")){
				...
				if(instance.getConfig().getBoolean("Config.Mail.Ativar")){
					...
					new SendMail(instance, p, 
							...
							transaction.getSender().getEmail(), transaction.getSender().getName(), 
							instance.getAssunto().replace("{player}", p.getName()).replace("{vip}", grupo).replace("{comprador}", transaction.getSender().getName()), 
							instance.getMensagem().replace("{player}", p.getName()).replace("{vip}", grupo)
							.replace("{tempo}", new SimpleDateFormat("dd/MM/yyyy").format(new Date(Long.parseLong(tempo))))
								.replace("{ativado}", sdf.format(Calendar.getInstance().getTime()))
								.replace("{comprador}", transaction.getSender().getName())
